...
export async function POST(req: NextRequest) {
  ...
  const idToken = cookieStore.get("p6_id_token")?.value;

  if (!idToken) {
    return NextResponse.json({ error: "Not authenticated — click 'Login to Run' first" }, { status: 401 });
  }

  const { message, session_id } = (await req.json()) as { message: string; session_id?: string };

  await emitEvent("Console", "triggering user-delegated mission", "P6 Orchestrator", "POST /chat — forwarding user id_token for XAA", undefined, "auth");
  ...
  if (!chatResp.ok || !chatResp.body) {
    ...
    await emitEvent("Console", "delegated mission failed", "P6 Orchestrator", err, undefined, "error");
