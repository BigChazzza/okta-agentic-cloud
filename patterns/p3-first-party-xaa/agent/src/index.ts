...
const HR_API_URL = process.env.PROTECTED_API_URL ?? "http://localhost:3101";
const FINANCE_API_URL = process.env.FINANCE_API_URL ?? "http://localhost:3102";
...
async function loadAgentConnections(): Promise<void> {
  ...
  // If Okta returned no XAA connections, fall back to static env var config.
  // This is the expected path when Managed Connections haven't been set up in the org.
  if (discoveredServers.length === 0) {
    activateFallback("no active XAA connections in Okta — using static env var config");
    return;
  }
