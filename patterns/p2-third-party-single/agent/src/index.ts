...
// crypto no longer needed — DCR/PKCE handled by the console
...
async function getMcpClient(accessToken?: string): Promise<{ client: Client; tools: ToolDef[] }> {
  ...
  const transport = new StreamableHTTPClientTransport(
    new URL(`${INVENTORY_SERVER_URL}/mcp`),
    { requestInit: { headers } }
  );
...
app.post("/chat", async (req, res) => {
  ...
  if (accessToken) {
    ...
    await emitEvent("Consumer Agent", "received access_token", "agent", "connecting to Inventory Server with user token", snippet, "token", accessToken);

    try {
      ({ client, tools } = await getMcpClient(accessToken));
    } catch (err) {
      const msg = err instanceof Error ? err.message : "MCP connection error";
      await emitEvent("Consumer Agent", "MCP connection failed", "Inventory Server", msg, undefined, "error");
      res.write(`Error connecting to Inventory Server: ${msg}`);
      res.end();
      return;
    }
    system = SYSTEM_WITH_TOOLS;
  } else {
    // Unauthenticated — public catalog mode; PKCE is handled entirely by the console
    await emitEvent("Consumer Agent", "received request", "agent", "no token — browsing mode (public tools only)", undefined, "info");

    const authLink = `[Authorize this agent](/api/auth/start/p2)`;
    await emitEvent("Consumer Agent", "authorization required", "User", "public mode, auth link provided", undefined, "auth");
