...
// XAA direct-access servers: populated from IDENTITY_ASSERTION_CUSTOM_AS connections.
// Used as fallback for user-delegated missions when A2A workers aren't configured.
interface XaaServer { label: string; url: string; audience: string; issuerUrl: string; }
let discoveredXaaServers: XaaServer[] = [];
...
async function loadA2AConnections(): Promise<void> {
  ...
  // Also extract IDENTITY_ASSERTION_CUSTOM_AS connections as direct XAA servers.
  // These allow the orchestrator to do user-delegated XAA without A2A workers.
  const xaaUrlMap: Record<string, { label: string; url: string }> = {};
  if (process.env.HR_API_URL && process.env.HR_RESOURCE_AUDIENCE)
    xaaUrlMap[process.env.HR_RESOURCE_AUDIENCE] = { label: "HR", url: process.env.HR_API_URL };
  if (process.env.FINANCE_API_URL && process.env.FINANCE_RESOURCE_AUDIENCE)
    xaaUrlMap[process.env.FINANCE_RESOURCE_AUDIENCE] = { label: "Finance", url: process.env.FINANCE_API_URL };

  discoveredXaaServers = connections
    .filter(c => c.connectionType === "IDENTITY_ASSERTION_CUSTOM_AS" && c.status === "ACTIVE")
    .flatMap(c => {
      const entry = c.resourceIndicator ? xaaUrlMap[c.resourceIndicator] : undefined;
      if (!entry || !c.authorizationServer?.issuerUrl) return [];
      return [{ label: entry.label, url: entry.url, audience: c.resourceIndicator!, issuerUrl: c.authorizationServer.issuerUrl }];
    });
  if (discoveredXaaServers.length > 0)
    console.log(`[Okta] Loaded ${discoveredXaaServers.length} XAA server(s) for user-delegated fallback: ${discoveredXaaServers.map(s => s.label).join(", ")}`);

  // If Okta returned no A2A worker connections, fall back to static env var config.
  if (discoveredWorkers.length === 0) {
    activateFallback("no active A2A worker connections in Okta — using static env var config");
    return;
  }
...
app.use((req, res, next) => {
  const _allowedOrigin = (process.env.CONSOLE_URL ?? "http://localhost:3020"); res.header("Access-Control-Allow-Origin", _allowedOrigin);
...
app.post("/chat", async (req, res) => {
  ...
  // Autonomous path (no user token): agent mints CC tokens directly — per P6 design.
  // User sign-on path: token forwarded from console login for user-delegated missions.
  const authHeader = req.headers.authorization;
  const userToken = authHeader?.startsWith("Bearer ") ? authHeader.slice(7) : null;

  if (!userToken) {
    const hrAuthzId  = process.env.HR_AUTHZ_SERVER_ID ?? "";
    const finAuthzId = process.env.FINANCE_AUTHZ_SERVER_ID ?? "";
    const hrApiUrl   = process.env.HR_API_URL ?? "http://localhost:3101";
    const finApiUrl  = process.env.FINANCE_API_URL ?? "http://localhost:3102";
    const ccClientId     = process.env.P6_OKTA_CLIENT_ID ?? "";
    const ccClientSecret = process.env.P6_OKTA_CLIENT_SECRET ?? "";

    if (!hrAuthzId || !finAuthzId || !ccClientId) {
      await emitEvent("P6 Orchestrator", "misconfigured", "agent", "HR_AUTHZ_SERVER_ID, FINANCE_AUTHZ_SERVER_ID, or P6_OKTA_CLIENT_ID not set", undefined, "error");
      res.status(500).json({ error: "P6 not configured — check HR_AUTHZ_SERVER_ID, FINANCE_AUTHZ_SERVER_ID, P6_OKTA_CLIENT_ID" });
      return;
    }

    await emitEvent("P6 Orchestrator", "autonomous mode", "Okta", `acquiring CC tokens — sub=${ccClientId}`, undefined, "auth");
  ...
  res.setHeader("X-Accel-Buffering", "no");
    ...
    try {
      const getCcToken = async (authzServerId: string, scope: string): Promise<string> => {
        const tokenUrl = `https://${OKTA_DOMAIN}/oauth2/${authzServerId}/v1/token`;
        const detail = `POST /oauth2/${authzServerId}/v1/token  grant_type=client_credentials  scope=${scope}  client_id=${ccClientId}`;
        await emitEvent("P6 Orchestrator", "CC grant", "Okta", detail, undefined, "auth");
        const resp = await fetch(tokenUrl, {
          method: "POST",
          headers: {
            "Content-Type": "application/x-www-form-urlencoded",
            Authorization: `Basic ${Buffer.from(`${ccClientId}:${ccClientSecret}`).toString("base64")}`,
          },
          body: new URLSearchParams({ grant_type: "client_credentials", scope }),
        });
        if (!resp.ok) throw new Error(`CC grant failed (${scope}): ${resp.status} ${await resp.text()}`);
        const { access_token } = await resp.json() as { access_token: string };
        const snippet = access_token.slice(0, 12) + "..." + access_token.slice(-8);
        await emitEvent("Okta", "issued CC token", "P6 Agent", `scope=${scope} sub=${ccClientId}`, snippet, "token", access_token);
        return access_token;
      };

      const [hrToken, finToken] = await Promise.all([
        getCcToken(hrAuthzId, "hr:read"),
        getCcToken(finAuthzId, "finance:read"),
      ]);

      const connectMcp = async (url: string, token: string, label: string) => {
        ...
        const client = new Client({ name: "p6-agent", version: "1.0.0" });
        await client.connect(transport);
        const { tools } = await client.listTools();
        await emitEvent("P6 Agent", "connected to MCP", label, `tools=${tools.length}`, undefined, "info");
        return { client, tools: tools.map(t => ({ ...t, _server: label })), token };
      };

      const [hrMcp, finMcp] = await Promise.all([
        connectMcp(hrApiUrl, hrToken, "HR Server"),
        connectMcp(finApiUrl, finToken, "Finance Server"),
      ]);

      const allTools = [...hrMcp.tools, ...finMcp.tools];
      const mcpToolDefs = allTools.map(t => ({
        name: t.name,
        description: (t as { description?: string }).description ?? "",
        inputSchema: ((t as { inputSchema?: unknown }).inputSchema ?? { type: "object", properties: {} }) as Record<string, unknown>,
      }));
      const toolDefs = [...mcpToolDefs, SLACK_TOOL];
      ...
      const callTool = async (name: string, args: Record<string, unknown>): Promise<string> => {
        ...
        await emitEvent("P6 Agent", "tool call", mcp.tools[0]?._server ?? "MCP", `tool=${name}`, undefined, "info");
        const result = await mcp.client.callTool({ name, arguments: args });
        return (result.content as Array<{ type: string; text?: string }>)
          .filter(c => c.type === "text").map(c => c.text).join("\n");
      };

      const autonomousSystem = `You are a fully autonomous AI agent (P6). You were triggered automatically — no user is present.
Use the available HR and Finance tools to fulfil the mission, compose a clear analysis, then post the final report to Slack using post_slack_message.
Token sub in all tool calls: ${ccClientId} (machine identity — no delegated user).`;

      const sid = session_id ?? "autonomous";
      const history = sessions.get(sid) ?? [];
      let fullResponse = "";

      for await (const chunk of runAnthropic([...history, { role: "user" as const, content: message }], autonomousSystem, callTool, toolDefs, llmOverrides)) {
        fullResponse += chunk;
        res.write(chunk);
      }

      await hrMcp.client.close().catch(() => {});
      await finMcp.client.close().catch(() => {});
      sessions.set(sid, [...history, { role: "user" as const, content: message }, { role: "assistant" as const, content: fullResponse }].slice(-20));
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Unknown error";
      await emitEvent("P6 Agent", "autonomous mission failed", "error", msg, undefined, "error");
      res.write(`Error: ${msg}`);
    }
    res.end();
    return;
  ...
  // If no A2A workers are configured but XAA servers are available, use direct XAA
  // (user-delegated identity flows from id_token → ID-JAG → resource token, same as P3).
  const useDirectXaa = discoveredWorkers.length === 0 && discoveredXaaServers.length > 0;
  if (useDirectXaa) {
    await emitEvent("P6 Orchestrator", "direct XAA mode", "agent", `no workers — using IDENTITY_ASSERTION_CUSTOM_AS for ${discoveredXaaServers.map(s => s.label).join(", ")}`, undefined, "auth");
  }

  try {
    if (useDirectXaa) {
      // ── User-delegated XAA: id_token → ID-JAG → resource token → MCP ──────
      const getXaaToken = async (server: XaaServer): Promise<string> => {
        // Step 1: id_token → ID-JAG at org server
        const jwtA = await buildAgentJwt(ORG_TOKEN_URL);
        const step1 = await fetch(ORG_TOKEN_URL, {
          method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" },
          body: new URLSearchParams({
            grant_type: "urn:ietf:params:oauth:grant-type:token-exchange",
            requested_token_type: "urn:ietf:params:oauth:token-type:id-jag",
            subject_token: userToken!, subject_token_type: "urn:ietf:params:oauth:token-type:id_token",
            audience: server.issuerUrl, scope: `${server.label.toLowerCase()}:read`,
            client_assertion_type: "urn:ietf:params:oauth:client-assertion-type:jwt-bearer",
            client_assertion: jwtA,
          }),
        });
        if (!step1.ok) throw new Error(`XAA Step1 (${server.label}): ${step1.status} ${await step1.text()}`);
        const { access_token: idJag } = await step1.json() as { access_token: string };
        // Step 2: ID-JAG → resource token at domain auth server
        const tokenUrl = `${server.issuerUrl}/v1/token`;
        const jwtB = await buildAgentJwt(tokenUrl);
        const step2 = await fetch(tokenUrl, {
          method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" },
          body: new URLSearchParams({
            grant_type: "urn:ietf:params:oauth:grant-type:jwt-bearer", assertion: idJag,
            client_assertion_type: "urn:ietf:params:oauth:client-assertion-type:jwt-bearer",
            client_assertion: jwtB,
          }),
        });
        if (!step2.ok) throw new Error(`XAA Step2 (${server.label}): ${step2.status} ${await step2.text()}`);
        const { access_token } = await step2.json() as { access_token: string };
        const snippet = access_token.slice(0, 12) + "..." + access_token.slice(-8);
        await emitEvent("Okta", `issued ${server.label} token`, "P6 Orchestrator", `aud=${server.audience} via XAA`, snippet, "token", access_token);
        return access_token;
      };

      const tokens = await Promise.all(discoveredXaaServers.map(s => getXaaToken(s)));

      const connectMcp = async (server: XaaServer, token: string) => {
        const { Client } = await import("@modelcontextprotocol/sdk/client/index.js");
        const { StreamableHTTPClientTransport } = await import("@modelcontextprotocol/sdk/client/streamableHttp.js");
        const transport = new StreamableHTTPClientTransport(new URL(`${server.url}/mcp`), { requestInit: { headers: { Authorization: `Bearer ${token}` } } });
        const client = new Client({ name: "p6-orchestrator", version: "1.0.0" });
        await client.connect(transport);
        const { tools } = await client.listTools();
        return { client, tools: tools.map(t => ({ ...t, _server: server.label })), server };
      };
      const mcpConns = await Promise.all(discoveredXaaServers.map((s, i) => connectMcp(s, tokens[i])));
      const allTools = mcpConns.flatMap(m => m.tools);
      const toolDefs = [...allTools.map(t => ({ name: t.name, description: (t as { description?: string }).description ?? "", inputSchema: ((t as { inputSchema?: unknown }).inputSchema ?? { type: "object", properties: {} }) as Record<string, unknown> })), SLACK_TOOL];

      const callToolXaa = async (name: string, args: Record<string, unknown>): Promise<string> => {
        if (name === "post_slack_message") return postSlackMessage(args.text as string, (args.channel as string | undefined) ?? slackChannel, slackToken);
        const conn = mcpConns.find(m => m.tools.some(t => t.name === name));
        if (!conn) return `Tool ${name} not found`;
      ...
      const xaaSystem = `You are a user-delegated AI agent (P6 Mission 2). The user has logged in and delegated their identity to you via Okta XAA. You have direct HR and Finance tools — use them to complete the mission, then post the report to Slack with post_slack_message. The user's identity flows in every token.`;
      let fullResponse = "";
      for await (const chunk of runAnthropic([...history, { role: "user" as const, content: message }], xaaSystem, callToolXaa, toolDefs, llmOverrides)) {
        fullResponse += chunk; res.write(chunk);
      }
      await Promise.all(mcpConns.map(m => m.client.close().catch(() => {})));
      sessions.set(sid, [...history, { role: "user" as const, content: message }, { role: "assistant" as const, content: fullResponse }].slice(-20));
    } else {
      let fullResponse = "";
